...
(function($, window, document, undefined) {
	...
	SliderVideos = {
		...
		bindEvents: function() {
			...
			$(document)
                        .on('builder_load_on_ajax builder_slider_videos_loaded', 'body', self.windowLoad)
                        .on(self.visibilityEventName, self.handleVisibilityChange)
			.focus(function(){self.handleVisibilityChange(true)})
			.blur(function(){self.handleVisibilityChange(false)});
			$(window)
                        .focus(function(){self.handleVisibilityChange(true)})
                        .blur(function(){self.handleVisibilityChange(false)});
		},
				...
				Themify.LoadAsync(
					...
					function() {
						Themify.LoadAsync(
								...
								Themify.LoadAsync(
									...
									function() {
										Themify.LoadAsync(
											...
											function() { self.makeSliderCallBack($slider) },
											null,
											null,
							...
							null,
							null,
							function() { return ('undefined' !== typeof videojs) }
						);
					},
                                        null,
                                        null,
		...
		sliderEvents: function() {
			var self = SliderVideos;
			$(document)
                        .off('click', '.tb_slider_videos_nav_control_play:not(.loading)', self.sliderEventsPlay)
			.on('click', '.tb_slider_videos_nav_control_play:not(.loading)', self.sliderEventsPlay)
			.off('click', '.tb_slider_videos_nav_control_pause:not(.loading)', self.sliderEventsPause)
                        .on('click', '.tb_slider_videos_nav_control_pause:not(.loading)', self.sliderEventsPause);
		},
