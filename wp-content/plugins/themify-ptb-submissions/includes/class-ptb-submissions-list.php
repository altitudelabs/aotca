    ...
    function process_bulk_action() {
        ...
        if ('delete' === $this->current_action() && current_user_can('manage_options')) {
            ...
            if (isset($_REQUEST[$this->_args['singular']]) && !empty($_REQUEST[$this->_args['singular']])) {
                ...
                if (count($names) > 0) {
                    $ptb_options->update();
                    $message = count($names) == 1 ? sprintf(__('%1$s submission template successfully removed.', 'ptb'), implode(', ', $names)) :
                            sprintf(__('%1$s submission templates successfully removed.', 'ptb'), implode(', ', $names));
